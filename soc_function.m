function Wsoc = soc_function(W_initial, rate, desired_SA, gamma, IEratio)
...
while e(i) > desired_SA
    ...
    Wsoc = ssaCode(Wsoc,rate,gamma,IEratio);  %Run one iteration of the gradient descent
...
function [Wo, Emax] = ssaCode(Wi,rate,gamma,IEratio)

N = length(Wi);         %Number of neurons
end_exc = N*(1-IEratio);          %Index of final excitatory neuron
start_inh = N*(1-IEratio) + 1;    %Index of first inhibitory neuron
